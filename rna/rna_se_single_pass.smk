...
if genomeVersion == "hg38":
    RRNA_BOWTIE_INDEX = (
        "/ds918_208/shared/genomics/FOR_PIPELINE/HG38/RRNA/human.ribo.rna"
    )
    ...
    RRNA_BOWTIE_INDEX = (
        "/ds918_208/shared/genomics/FOR_PIPELINE/MM10/RRNA/mouse.ribo.rna"
    )
...
rule all:
    input:
        ...
        transcriptomeBam=expand(
            OUT + "/bam/star/{sample}.Aligned.toTranscriptome.out.bam",
            sample=sampleList,
        ),
        genomeBam=expand(
            OUT + "/bam/star/{sample}.Aligned.sortedByCoord.out.bam", sample=sampleList
        ),
        genomeBai=expand(
            OUT + "/bam/star/{sample}.Aligned.sortedByCoord.out.bam.bai",
            sample=sampleList,
        ),
...
def cut_if_adapter(wildcards):
    ...
    r_adap = (
        subprocess.check_output(f"cat {r_adap} | sed -n 9p | cut -f 3", shell=True)
        .decode("utf-8")
        .strip()
    )
    if any([r_adap == ""]):
        ...
        return {
            "fq": OUT + "/fastq/{sample}.trimmed.fastq.gz",
        }
...
rule cutAdapter:
    input:
        fq=IN + "/{sample}.fastq.gz",
        adp=rules.detectAdapter.output.adapter,
    output:
        fq=OUT + "/fastq/{sample}.trimmed.fastq.gz",
    ...
    shell:
        """
        adaptor_seq=$(cat {input.adp} |sed -n 9p |cut -f 3 )
...
rule rrna:
    ...
    run:
        ...
        print(bowtie2Log)
        t1 = re.search("(\d+) \((.*)\) aligned exactly 1 time", bowtie2Log)
...
rule summaryConcat:
    ...
    run:
        ...
        def writeInfo(i, fileHandle):
            ...
            starLogDict = dict(
                [x for x in [re.split("\ \|\t", x) for x in starLogDict] if len(x) == 2]
            )
...
rule countMatrix:
    ...
    run:
        ...
        nameList = [
            re.search("(?<=\/rsem\/).*(?=.genes.results)", x).group(0) for x in pathList
        ]
        ...
        countDF.to_csv(
            output.rawCount, sep="\t", index=True, header=True, float_format="%.2f"
        )
        ...
        fpkmDF.to_csv(
            output.fpkm, sep="\t", index=True, header=True, float_format="%.2f"
        )
